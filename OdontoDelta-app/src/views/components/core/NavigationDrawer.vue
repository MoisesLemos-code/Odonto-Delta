<template>
  <v-navigation-drawer :mini-variant='drawerControl' dark>
    ...
    <template v-slot:append>
      <v-btn @click.stop="gerenciarGavetaLateral" block tile>
        <v-icon v-if="drawerControl">mdi-arrow-right-bold-box-outline</v-icon>
        <div v-else>
          <v-icon>mdi-arrow-left-bold-box-outline</v-icon>
          <label>Retrair menu</label>
        </div>
      </v-btn>
    </template>
  </v-navigation-drawer>
...
export default {
  ...
  computed: {
    drawerControl: function () {
      return this.$store.state.miniMenuLateral
    }
  },
  methods: {
    gerenciarGavetaLateral() {
      if (this.$store.state.miniMenuLateral) {
