<template>
  <div>
    <v-btn
      ...
      style="float: left; margin: 4% 1% -2%"
      @click="comfirmarInserirMesa"
    >
      inserir
      mesa
    </v-btn>
          ...
          <v-flex lg2 md2>
            <v-hover>
              <template v-slot="{ hover }">
                <v-card
                  @click="buscarPedidoPorMesa(mesa)"
                ...
                >
                  <span
                    style="font-size: 30px; display: flex; justify-content: center; letter-spacing: 2px; font-weight: bold;"
                  >{{mesa.numero}}</span>
                </v-card>
              </template>
            </v-hover>
    ...
    />
    <notificacao />
    <atencao :dialog="dialog" :mensagem="mensagem" @cancelar="cancelar" @confirmar="confirmar" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async buscarPedidoPorMesa(mesa) {
      let comanda = await this.$store.dispatch(
        actionTypes.BUSCAR_PEDIDO_POR_MESA,
        mesa.id
      );
      if (comanda) {
    ...
    comfirmarInserirMesa() {
      ...
      this.dialog = true;
      this.mensagem =
        "Deseja mesmo inserir uma nova mesa? após inserir não há como excluir";
    },
    cancelar() {
      this.dialog = false;
